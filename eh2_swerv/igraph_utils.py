...
def netlist2igraph(directed = False, multiple_edges = False , inputs_as_verticies = True, top = None, clean_graph = True, debug = False):
    ...
    g.add_vertices(num_of_gates + num_of_submodules + num_of_inputs)
    vid = 0
    ...
    for gate in gates:
        ...
        g.vs[vid]["celltype"] = gate.type.name
        g.vs[vid]["netid"] = -1
    ...
    # phase 1.5 - dealing with vertices representing submodules
    sid_to_vid = {}
    for submodule in submodules:
        g.vs[vid]["label"] = submodule.name
        g.vs[vid]["celltype"] = submodule.type
        g.vs[vid]["netid"] = -1
        g.vs[vid]["gateid"] = submodule.id
        assert submodule.id not in sid_to_vid, "submodule_id="+str(submodule.id)+" already exists in gid_to_vid!"
        sid_to_vid[submodule.id] = vid
        vid += 1
    ...
    if inputs_as_verticies:
        for net in inputs_nets:
            g.vs[vid]["label"] = "NET___" + net.get_name()
            g.vs[vid]["celltype"] = "DUMMY_NET"
        ...
        for net in inputs_nets:
            for dest in net.destinations:
                ...
                if dest.gate.module.name != top.name:
                    # if dest is in another submodule
                    v2 = sid_to_vid[dest.gate.module.id]
                else:
    ...
    for net in top.get_internal_nets():
        ...
        for dest in net.destinations:
            ...
            # if dest is another submodule
            if net.sources[0].gate.module.name != top.name:
                v1 = sid_to_vid[net.sources[0].gate.module.id]
            else:
